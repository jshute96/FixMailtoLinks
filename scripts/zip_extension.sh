...
#
# Default output:           /tmp/FixMailtoLinks.zip
# With --release VERSION:   /tmp/FixMailtoLinks-vVERSION.zip
#
# The versioned name is used by scripts/release-extension.sh so the
# downloaded release artifact is self-identifying.
...
Options:
  --release VERSION   Tag the zip with VERSION
                      (/tmp/FixMailtoLinks-vVERSION.zip).
                      Default is /tmp/FixMailtoLinks.zip.
...
if [[ -n "$VERSION" ]]; then
  TARGET="/tmp/FixMailtoLinks-v${VERSION}.zip"
else
